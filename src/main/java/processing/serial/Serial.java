...
public class Serial implements SerialPortEventListener {
	...
	public interface SerialListener {
		public void onEvent(SerialPortEvent event, Serial port);
	}
	...
	public boolean getCTS() {
		try {
			...
			throw new RuntimeException("Error reading the CTS line: "
					+ e.getExceptionType());
		}
	}
	...
	public boolean getDSR() {
		try {
			...
			throw new RuntimeException("Error reading the DSR line: "
					+ e.getExceptionType());
		}
	}
	...
	public int readBytesUntil(int inByte, byte[] dest) {
		...
		synchronized (buffer) {
			...
			if (dest.length < toCopy) {
				System.err
						.println("The buffer passed to readBytesUntil() is to small "
								+ "to contain "
								+ toCopy
